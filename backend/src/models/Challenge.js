...
class Challenge {
  ...
  static async create(challengeData) {
    try {
      const { title, description, difficulty, subject, points, type, content, created_by, goal_id } = challengeData;
      const query = `
        INSERT INTO challenges (title, description, difficulty, subject, points, type, content, created_by, goal_id, created_at, updated_at)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW(), NOW())
        RETURNING *
      `;
      const result = await db.query(query, [title, description, difficulty, subject, points, type, content, created_by, goal_id]);
      return result.rows[0];
    } catch (error) {
  ...
  static async update(challengeId, updateData) {
    try {
      ...
      const query = `
        ...
        RETURNING *
      `;
      const result = await db.query(query, [challengeId, title, description, difficulty, subject, points, type, content, goal_id]);
      return result.rows[0];
    } catch (error) {
